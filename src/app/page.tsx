...
async function getRaces(year: number) {
  try {
    ...
    const res = await fetch(`https://f1api.dev/api/${slug}`, {
      next: { revalidate: 300 },
    })
...
export default async function HomePage({
  ...
  const nextRace = !isPastSeason
    ? races.find((r: any) => r.schedule?.race?.date && new Date(r.schedule.race.date) >= today)
    : null
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <YearSelector selectedYear={year} years={AVAILABLE_YEARS} basePath="/" currentYear={CURRENT_YEAR} />

      {nextRace && <NextRaceCard race={nextRace} today={today} year={year} />}
      ...
      <div className="bg-zinc-900 rounded-xl border border-zinc-800 overflow-hidden">
        {races.map((race: any) => {
          ...
          const isPast = isPastSeason || (!!raceDate && raceDate < today)
          const isNext = !isPastSeason && !!nextRace && race.round === nextRace.round
          return (
            <RaceRow key={race.round} race={race} isPast={isPast} isNext={isNext} year={year} />
          )
        })}
      </div>
    </div>
...
function NextRaceCard({ race, today, year }: { race: any; today: Date; year: number }) {
...
function RaceRow({ race, isPast, isNext, year }: { race: any; isPast: boolean; isNext: boolean; year: number }) {
  const flag = COUNTRY_FLAGS[race.circuit?.country] ?? '🏁'
  const raceDate = race.schedule?.race?.date ? new Date(race.schedule.race.date) : null
  const winner = race.winner
  const hasSprint = !!race.schedule?.sprint?.date
  const href = year !== CURRENT_YEAR ? `/race/${race.round}?year=${year}` : `/race/${race.round}`
